...
re_episode_oad = re.compile(r"OAD(?:\s*|\.)(\d+)")
re_episode_ncop = re.compile(r"NCOP(?:\s*|\.)(\d+)")
re_episode_nced = re.compile(r"NCED(?:\s*|\.)(\d+)")
re_episode_extra = re.compile(r"extra(?:\s*|\.)(\d+)", re.IGNORECASE)
re_episode = re.compile(
    r"(?:^|[^a-zA-Z]+)(?:episode|e|ep)(?:\s*|\.)(\d+)",
    re.IGNORECASE
)
...
re_year = re.compile(
    r"\s*(?:\((\d{4})\)|\[(\d{4})\]|(?:\s+|\.)(\d{4})(?:\s+|\.))\s*"
)
re_usertags = re.compile(r"\[[^\]]*\]\s*")
re_clean_show_name = (
    re.compile(r"(?:720|1080)p(?:\s+|\.|-|$)", re.IGNORECASE),
    # re.compile("720p(?:\s+|\.|-|$)", re.IGNORECASE),

    re.compile(r"(?:web|hd|bd)rip(?:\s+|\.|$)", re.IGNORECASE),
    # re.compile("hdrip(?:\s+|\.|$)", re.IGNORECASE),
    re.compile(r"web-dl(?:\s+|\.|$)", re.IGNORECASE),

    re.compile(r"\d+mb(?:\s+|\.|-|$)", re.IGNORECASE),

    re.compile(r"x26(?:4|5)(?:\s+|\.|-|$)", re.IGNORECASE),
    re.compile(r"hvec(?:\s+|\.|-|$)", re.IGNORECASE),
    re.compile(r"xvid(?:\s+|\.|-|$)", re.IGNORECASE),
    re.compile(r"aac(?:\s+|\.|-|$)", re.IGNORECASE),
    re.compile(r"ac3(?:.?(?:evo))?(?:\s+|\.|-|$)", re.IGNORECASE),
    re.compile(r"aac5.1(?:\s+|\.|-|$)", re.IGNORECASE),
...
def is_oad(file):
    return re_oad.search(remove_user_tags(remove_meta_tags(file))) is not None


def is_extra(file):
    file_removed_tags = remove_user_tags(remove_meta_tags(file))
    return re_extra.search(file_removed_tags) is not None


def is_ncop(file):
    return re_ncop.search(remove_user_tags(remove_meta_tags(file))) is not None


def is_nced(file):
    return re_nced.search(remove_user_tags(remove_meta_tags(file))) is not None
